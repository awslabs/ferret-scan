...
| TM-10 | Dockerfile builder-stage `FROM` not pinned by digest (TB-8) | Minor | **Mitigated** — added 2026-05-22 by REPO scan; PR #70 introduced digest pinning. The builder is now pinned to `golang:1.26.6-alpine@sha256:af8d6740070b8906d12eae1c3e3ea0957fb63f492051ea05e354c38ef9fe88df` (Go 1.26.6 picks up fixes for CVE-2026-46600 and CVE-2026-39821, both rated HIGH against the 1.26.5 stdlib and both flagged by Trivy on `main`; it also carries the earlier 1.26.5 fixes for CVE-2026-39822 and CVE-2026-42505). The tag+digest are kept in sync via `scripts/go-version.sh` from the `.go-version` source of truth; the digest references a multi-arch manifest list covering both `linux/amd64` and `linux/arm64`, matching the platforms `docker-multiarch.yml` builds. Final stage is `FROM scratch` and is unaffected. |
