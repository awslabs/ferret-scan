...
- **Honest check list**: every name `redact.ValidCheckNames()` returns can actually produce a finding on this in-memory path — that is enforced by a test that drives a positive fixture through `Redact` for each one and asserts the output changed. Two validators are therefore *excluded*: `METADATA` (needs filesystem access) and `SOCIAL_MEDIA` (ships no built-in patterns; platform patterns come only from a config file, which this API does not accept). Naming only those returns a `"no validators enabled"` error rather than an engine that silently detects nothing. Use `pkg/scan` — which loads project config — if you need social-media detection.
