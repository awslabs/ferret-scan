...
| ~~45 SSN false positives~~ | **Partly resolved 2026-08-28 (#389).** The header was consulted only as a NEGATIVE signal from a fixed ~45-word list, so `tracking_number` / `order_id` / `zip_code` were demoted while an identifier column nobody had listed — `parcel_id`, `meter_number`, `record_id` — still reached HIGH. A second arm now tests the header's SHAPE: a `<qualifier>_id`/`_number`/`_no` header caps at 55 unless the qualifier names an identity document or a person. Measured on the corpus: **FP(H+M) 12 → 9 with the recall floor untouched** (`tp_high_medium` 154, `fn_missed` 0, `fn_band` 0); the three moved to `fp_low`. The person half is the corpus's own answer, not a guess — `c19_honest_member_id` and `c20_honest_participant_id` are labelled BandHigh, so `member_id` must stay HIGH while `parcel_id` must not. |
